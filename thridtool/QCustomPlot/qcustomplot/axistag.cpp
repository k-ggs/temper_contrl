  ...
  mAxis(parentAxis)
{
  ...
  mDummyTracer->setVisible(false);//anchor is line visible false
  ...
  // mLabel->setBrush(QBrush(Qt::NoBrush));
...
// mLabel->setPositionAlignment(Qt::AlignTop);

  //this title of label
   mTitle = new QCPItemText(mAxis->parentPlot());
   mTitle->setLayer("overlay");
   mTitle->setClipToAxisRect(false);
   mTitle->setPadding(QMargins(3, 0, 3, 0));
   // mLabel->setBrush(QBrush(Qt::white));
   mTitle->setBrush(QBrush(Qt::white));
   mTitle->setBrush(QBrush(Qt::NoBrush));

   // mLabel->setPen(QPen(Qt::blue));
    QFont font2;
    font.setPixelSize(15);
   mTitle->setFont(font2);
   mTitle->setPen(QPen(Qt::NoPen));
   //mTitle->setTextAlignment()
   mTitle->setPositionAlignment(Qt::AlignTop);

   mLabel->position->setParentAnchor(mArrow->start);


 //   mLabel->position->setParentAnchor(mArrow->start);
      mTitle->position->setParentAnchor(mLabel->bottomLeft);
 //   mTitle->position->setCoords(mArrow->start->key(),0);
}
...
void AxisTag::setTitle(const QString &text)
{
  mTitle->setText(text);
}
